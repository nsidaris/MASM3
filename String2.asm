...
; Date:          April 16, 2017
; Purpose:
;        Define the methods for the String class: (String2.asm)
...
****************************************************************************************%
;commented out, use method from string1.asm to avoid conflicts
...
String_indexOf1 proc Near32
	...
	push ebx
	call String_length		;check length of string
	add esp,4
	cmp eax,0
	je charNotFound			;if empty string(cant find char, exit and retunr -1)
...
String_indexOf3 proc Near32
	...
	add esp,4
	cmp eax,0
	je strEnd			;if empty string(cant find substring, exit and retunr -1)
...
String_toLowerCase proc Near32
	...
	push ebx			;get length of string
	call String_length
	add esp,4
	cmp eax,0
	je empty			;if string emty retunr -1 in eax
...
* Name: String_replace																	*
* Purpose:																				*
*		Replaces all oldChars in a string with NewChars									*
*Changes are made directly to the string (nothing is returned in a register)			*
*																						*
...
String_replace proc Near32
	...
	push [ebp+8]
	call String_length		;checks if the string is empty
	add esp,4
	cmp eax,0
	je emptyStr
...
String_lastIndexOf1 proc Near32
	...
	add esp,4			;fix the stack
	cmp eax,0
	je notFound			;empty string return -1 in eax
...
String_lastIndexOf2 proc Near32
	...
	add esp,4
	cmp eax,0
	je charNotFound	;empty string, return -1
...
subLoop:
	...
	mov al, byte ptr[ebx+esi]	;store value(char) from string
	inc esi	
	cmp byte ptr [ecx+edx],al	;compare value in substring with stroed value form string
...
found:
	...
	mov dVal2,eax
	jmp stLoop		;continue loop until end of string
exit:
...
String_concat proc Near32
	...
	cmp eax,0
	je fail		;both strings empty, return -1
	...
	je fail	;cant allocate memory returns -1 in eax
...
addNull:
	...
	jmp exit
fail:
	mov eax,-1					;couldnt allocate memory from heap, returns 0
exit:
	pop edx
	pop ecx
	pop esi			;restore preserved registers
